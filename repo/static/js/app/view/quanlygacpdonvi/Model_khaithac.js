define(function (require) {
	...
	var template 				= require('text!app/view/quanlygacpdonvi/tpl/model_khaithac.html'),
    ...
    return Gonrin.ModelView.extend({
    	...
    	tools : [
						...
						if (delete_success == true) {
							...
							self.model.destroy({
								success: function(model, response) {
									...
									let path = 'quanlygacp_khaithac/collection';
			...
			},
			{
				...
				label: `<i class="fas fa-mail-bulk"></i> Gửi duyệt`,
				visible: function(){
					return (this.getApp().getRouter().getParam("id") !== null && (this.getApp().hasRole("admin_donvi") || this.getApp().hasRole("canbo")));
				},
				command: function(){
		...
		validateData: function(){
			...
			if (!diadiem){
				self.getApp().notify({message: "Vui lòng nhập địa điểm khai thác"}, {type : "danger", delay : 1000});
				return false;
			}
