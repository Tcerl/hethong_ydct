define(function (require) {
    ...
    var template 				= require('text!app/danhmuc/danhmuc_sanpham/CayThuoc/tpl/model.html'),
    	schema 				= require('json!schema/DMCayThuocSchema.json');
    ...
    return Gonrin.ModelDialogView.extend({
        		...
        		this.model.fetch({
        			success: function(data) {
						...
						if(!gonrinApp().hasRole("admin") && tuyendonvi_id != '1'){
							...
							self.$el.find("#button_off").hide();
						}
						var fileView = new ManageFileVIew({ viewData: { "listFile": self.model.get('hinh_anh'), "el_btn_upload": ".btn-add-file" }, el: self.$el.find(".list-attachment") });
                        fileView.render();
                        fileView.on("change", (event) => {
                            var listfile = event.data;
                            self.model.set('hinh_anh', listfile);
                        });
		...
		validateData: function() {
			var self = this;
			if (!self.model.get('ma_caythuoc')) {
				self.getApp().notify({ message: "Chưa nhập mã cây thuốc"}, { type: "danger", delay: 1000 });
				return false;
			}
			if (!self.model.get('ten_caythuoc')) {
				self.getApp().notify({ message: "Chưa nhập tên cây thuốc"}, { type: "danger", delay: 1000 });
				return false;
			}
			return true;
		},
