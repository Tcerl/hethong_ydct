define(function(require) {
    ...
    return Gonrin.CollectionView.extend({
        ...
        modelSchema: schema,
        urlPrefix: "/api/v1/",
        collectionName: "danhmuc_duoclieu_filter",
        uiControl: {
            fields: [{
                    field: "stt",
                    label: "STT",
            ...
            onRowClick: function(event) {
                if (event.rowId) {
                    var path = 'danhmuc_duoclieu/view?id=' + event.rowId;
            ...
            buttons: [
                {
                    name: "create",
                    type: "button",
                    buttonClass: "btn-primary btn-sm btn-create",
                    label: `<i class="fa fa-plus"></i> Thêm mới`,
                    command: function() {
                        let path = `danhmuc_duoclieu/model`;
                ...
                },
                {
                    name:"downloadExcel",
                    type: "button",
                    buttonClass: "btn-success btn-sm ml-1 btn-template",
        ...
        render: function() {
            ...
            if(!gonrinApp().hasRole("admin") && tuyendonvi_id != '1'){
                self.$el.find(".btn-create").remove();
                self.$el.find(".btn-template").remove();
            }
            // } else {
            //     self.$el.find(".btn-create").remove();
            //     self.$el.find(".btn-template").remove();
