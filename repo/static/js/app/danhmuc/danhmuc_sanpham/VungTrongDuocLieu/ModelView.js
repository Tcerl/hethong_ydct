define(function (require) {
    ...
    var template 				= require('text!app/danhmuc/danhmuc_sanpham/VungTrongDuocLieu/tpl/model.html'),
    	schema 				= require('json!schema/DMVungTrongDuocLieuSchema.json');
    ...
    return Gonrin.ModelDialogView.extend({
		...
		validateData: function() {
			var self = this;
			if (!self.model.get('ma_vungtrong')) {
				self.getApp().notify({ message: "Chưa nhập mã vùng trồng"}, { type: "danger", delay: 1000 });
				return false;
			}
			if (!self.model.get('ten_vungtrong')) {
				self.getApp().notify({ message: "Chưa nhập tên vùng trồng"}, { type: "danger", delay: 1000 });
				return false;
			}
			return true;
		},
