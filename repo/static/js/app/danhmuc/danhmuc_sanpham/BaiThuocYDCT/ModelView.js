define(function (require) {
    ...
    var template 				= require('text!app/danhmuc/danhmuc_sanpham/BaiThuocYDCT/tpl/model.html'),
    	schema 				= require('json!schema/BaiThuocYDCTSchema.json');
    ...
    return Gonrin.ModelDialogView.extend({
    		...
    		if (id) {
        		...
        		this.model.fetch({
        			success: function(data) {
						...
						gonrinApp().selectizeTinhThanhQuanHuyenXaPhuong(self, false, false, false, '#selectize-tinhthanh', '#selectize-quanhuyen', '#selectize-xaphuong', 'tacgia_tinhthanh_id', 'tacgia_quanhuyen_id', 'tacgia_xaphuong_id', 'tacgia_tinhthanh', 'tacgia_quanhuyen', 'tacgia_xaphuong');
        			},
        			error:function() {
		...
		validateData: function() {
			var self = this;
			if (!self.model.get('ma_baithuoc')) {
				self.getApp().notify({ message: "Chưa nhập mã bài thuốc"}, { type: "danger", delay: 1000 });
				return false;
			}
			if (!self.model.get('ten_baithuoc')) {
				self.getApp().notify({ message: "Chưa nhập tên bài thuốc"}, { type: "danger", delay: 1000 });
				return false;
			}
			return true;
		},
