define(function (require) {
    ...
    var template = require('text!app/quanly_tintuc/baiviet/tpl/dialog_box_related_news.html');    
    var DialogChooseType = require('app/quanly_tintuc/baiviet/DialogChooseType');
    return Gonrin.ModelDialogView.extend({
		...
		tools: [
    	    ...
    	    },
            {
                ...
                label: `<i class="far fa-save"></i> Chọn kiểu hiển thị`,
                command: function() {
                    var self = this;
                    var list_post = self.model.get("related_news");
                    var type = self.model.get("type");
                    if (list_post instanceof Array && list_post.length > 0) {
                        var model_dialog = new DialogChooseType({"viewData": {"list_post": list_post, "type": type}});
    	...
    	render: function() {
            ...
            if (curUser) {
                ...
                var box_related_news_id = self.viewData.box_related_news_id;
                var type = self.viewData.type;
                if (box_related_news_id) {
                    self.model.set("box_related_news_id", box_related_news_id);
                }
                if (type) {
                ...
                if (related_news instanceof Array && related_news.length > 0) {
                    ...
                    for (var i = 0; i < related_news.length; i++) {
                        self.renderPostAdded(related_news[i]);
                    }
                }
        ...
        renderPostAdded: function(post){
            var self = this;
            self.$el.find("#div_post_added").append(`<div class="list_post_added" id="insert_` + post.id + `"><p style="margin-bottom:5px;">` + post.title + `</p><span class="custom_close" id="delete_` + post.id + `" title="Xóa">&times;</span></div>`);
