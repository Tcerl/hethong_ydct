...
define(function (require) {
    ...
    var template 			= require('text!app/quanlykho/PhieuKiemKe/tpl/newmodel.html');
    ...
    return Gonrin.CollectionView.extend({
		...
		render:function() {
			...
			if (!!id) {
				$.ajax({
					...
					success: function(response) {
						...
						var fileView = new ManageFileVIew({viewData:{"listFile": self.array_chungtu, "$el_btn_upload": self.$el.find(".btn-add-file")}, el:self.$el.find(".list-attachment")});
						fileView.render();
				...
				self.event_selectize();
				var fileView = new ManageFileVIew({viewData:{"listFile": [], "$el_btn_upload": self.$el.find(".btn-add-file")}, el:self.$el.find(".list-attachment")});
				fileView.render();
				fileView.on("change", (event) => {
					var listfile = event.data;
					self.array_chungtu = listfile;
			...
			self.$el.find(".btn-xoaboloc").unbind("click").bind("click", function (e) {
				e.stopPropagation();
				e.preventDefault();
                let selectkho = (self.$el.find("#chonkho"))[0];
                if (!!selectkho && !!selectkho.selectize){
                    selectkho.selectize.clear();
                    self.kho_id = null;
                }
