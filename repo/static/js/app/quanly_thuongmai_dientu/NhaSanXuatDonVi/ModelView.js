define(function (require) {
    ...
    var template = require('text!app/quanly_thuongmai_dientu/NhaSanXuatDonVi/tpl/model.html');
    var schema = require('json!schema/NhaSanXuatDonViSchema.json');
    var RejectDialogView = require('app/bases/RejectDialogView');
    return Gonrin.ModelDialogView.extend({
        template: template,
        modelSchema: schema,
        urlPrefix: "/api/v1/",
        collectionName: "nhasanxuat_donvi",
        tools: [
            {
                name: "back",
                type: "button",
            ...
            if (id) {
                ...
                this.model.fetch({
                    success: function () {
                        ...
                        var nhasanxuat_id = self.model.get("nhasanxuat_id");
        ...
        validateData: function () {
            ...
            if (ma == null || ma.trim() === "") {
                self.getApp().notify({ message: "Chưa nhập mã nhà sản xuất " }, { type: "danger", delay: 1000 });
                return false;
            }
            if (ten == null || ten.trim() === "") {
                self.getApp().notify({ message: "Chưa nhập tên nhà sản xuất " }, { type: "danger", delay: 1000 });
                return false;
            }
        ...
        selectizeNhaSanXuat: function () {
            var self = this;
            var firstLoad = true;
            var $selectize = self.$el.find('#selectize_nhasanxuat').selectize({
                maxItems: 1,
                valueField: 'id',
                labelField: 'ten',
                searchField: ['ten', 'tenkhongdau', 'ma'],
                preload: true,
                placeholder: "Chọn nhà sản xuất ",
                load: function (query, callback) {
                    if (firstLoad) {
                        firstLoad = false;
                        if (!!self.model.get("nhasanxuat_id") && self.model.get("nhasanxuat")) {
                            callback([self.model.get("nhasanxuat")]);
                            self.$el.find("#selectize_nhasanxuat")[0].selectize.setValue(self.model.get("nhasanxuat_id"));
                        }
                    }
                    ...
                    var url = (self.getApp().serviceURL || "") + '/api/v2/nhasanxuat?page=1&results_per_page=15' + (query_filter ? "&query_filter=" + query_filter : "");
                    var params = {
                        page: 1,
                        per_page: 15
                    };
                ...
                onItemAdd: function (value) {
                    ...
                    var currentData = self.model.get("nhasanxuat");
                    ...
                    if (!currentData || currentData.id !== value) {
                        if (!!selectedData) {
                            ...
                            self.model.set({
                                ...
                                'trangthai':selectedData.trangthai,
                                ...
                                'nhasanxuat_id': selectedData.id,
                                'nhasanxuat': selectedData,
                            });
