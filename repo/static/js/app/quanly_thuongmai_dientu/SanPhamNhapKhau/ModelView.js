define(function (require) {
    ...
    var template 				= require('text!app/quanly_thuongmai_dientu/SanPhamNhapKhau/tpl/model.html'),
    	schema 				= require('json!schema/SanPhamNhapKhauDonViSchema.json');
    ...
    return Gonrin.ModelDialogView.extend({
    	...
    	modelSchema	: schema,
    	urlPrefix: "/api/v1/",
    	collectionName: "sanpham_nhapkhau_donvi",
    	tools : [
			{
				name: "back",
				type: "button",
		...
		validateData: function() {
			...
			if (!quocgia_id) {
				self.getApp().notify({ message: "Vui lòng chọn nơi cung cấp"}, { type: "danger", delay: 1000 });
				return false;
			}
		...
		selectizeNoiCungCap: function() {
			var self = this;
			var first_load = true;
			var $selectize = self.$el.find('#selectize_noi_cungcap').selectize({
                maxItems: 1,
                valueField: 'id',
                labelField: 'ten',
                searchField: ['ten', 'tenkhongdau'],
                preload: true,
				placeholder: "Chọn nơi cung cấp",
                load: function(query, callback) {
					if (first_load && !!(self.model.get("quocgia_id"))) {
						first_load = false;
						var quocgia_id = self.model.get("quocgia_id"),
							quocgia = self.model.get("quocgia");
						callback([{'id': quocgia_id, 'ten': quocgia.ten, 'tenkhongdau': quocgia.tenkhongdau, 'ten_tieng_anh': quocgia.ten_tieng_anh}]);
                        $selectize[0].selectize.setValue(quocgia_id);
					}
                    var query_filter = {
                        "filters": {
                            "$and": [
								...
								{ "$or": [
									{ "tenkhongdau": { "$likeI": gonrinApp().convert_khongdau(query) } },
									{ "ten_tieng_anh": { "$likeI": query } }
								]} 
                            ],
                            "order_by": [{ "field": "ten", "direction": "asc" }]
                        }
                    };
                    var url = (self.getApp().serviceURL || "") + '/api/v1/quocgia?page=1&results_per_page=15' + (query_filter ? "&q=" + JSON.stringify(query_filter) : "");
                    $.ajax({
                        url: url,
                        type: 'GET',
                        dataType: 'json',
                ...
                onItemAdd: function(value, $item) {
					if (value != self.model.get("quocgia_id")) {
						var obj = $selectize[0].selectize.options[value];
						delete obj.$order;
						self.model.set({
							"quocgia_id": value,
