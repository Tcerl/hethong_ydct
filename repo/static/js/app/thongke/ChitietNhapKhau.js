define(function (require) {
    ...
    var template 				= require('text!app/thongke/tpl/chitiet_nhapkhau.html');
    var NhapKhauDialog = require('app/giayphepnhapkhau/ModalViewData');
    ...
    return Gonrin.ModelDialogView.extend({
        ...
        showData: function(list_baocao){
            var self = this;
            if (Array.isArray(list_baocao) === true && list_baocao.length >0){
                        ...
                        for (let j=0; j<list_sanpham.length; j++){
                            ...
                            if (j==0){
                                ...
                                if (!!id_giay_phep && id_giay_phep !== ""){
                                    td_so_giay_phep = `<td><a class="giayphep" id_giay_phep="${id_giay_phep}" href="javascript:void(0)">` +so_giay_phep + ` </a></td>`;
                                }
                                else{
                                    td_so_giay_phep = `<td>` + (so_giay_phep? so_giay_phep : "") + `</td>`;
                                }
                                var td_so_luong_capphep = `<td class ="text-right">` + (so_luong_capphep? Number(so_luong_capphep).toLocaleString("de-DE") : 0) + `</td>`;
                                var td_so_luong_nhap = `<td class ="text-right">` + (so_luong_nhap? Number(so_luong_nhap).toLocaleString("de-DE") : 0) + `</td>`;
                                ...
                                for (var k=0;k<so_co.length;k++){
                                    ...
                                    if (!!chungnhan_co_id[k] && chungnhan_co_id[k] !== ""){
                                        tmp_td_so_co = `<div><a class="chungnhanco" chungnhan_co_id="${chungnhan_co_id[k]}" href="javascript:void(0)">` +so_co[k] + ` </a></div>`;
                                    }
                                    else{
                ...
                let giayphep_a = self.$el.find("a.giayphep");
                let length_a = giayphep_a.length;
                for (let i=0; i<length_a; i++){
                    let item_a = giayphep_a[i];
                    let id_giay_phep = $(item_a).attr("id_giay_phep");
                    $(item_a).unbind("click").bind("click", {data: id_giay_phep}, (e)=>{
                        if (!!e){
                            e.stopPropagation();
                            let tmp = e.data.data;
                            let giayphep_nhapkhau = new NhapKhauDialog({viewData: {"id" :  tmp}});
                            giayphep_nhapkhau.dialog({size: "large"});
                        }
                    });
                }
