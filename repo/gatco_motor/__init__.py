...
class Motor(object):
    ...
    def init_app(self, app, open_listener='before_server_start',
        ...
        if(uri is None):
            app.config['MOTOR_URI'] = app.config.get('MOTOR_URI', 'mongodb://127.0.0.1:27017/manage_file')
            print("app.config['MOTOR_URI']====uri None===", app.config['MOTOR_URI'])
        else:
            print("app.config['MOTOR_URI']====uri not None===", uri)
            app.config['MOTOR_URI'] = uri
